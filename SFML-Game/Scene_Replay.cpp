...
void Scene_Replay::resetScene()
{

}

void Scene_Replay::spawnPlayer()
{
...
void Scene_Replay::sEnemySpawner(int x, int y)
{
...
void Scene_Replay::registerAction(int code, const std::string& s)
{
	getActionMap()[code] = s;
}

void Scene_Replay::init()
{
	int m_heartNum = 3;
	int m_goldNum = 0;
	int m_bulletNum = 10;
...
void Scene_Replay::sDoAction(const Action& action)
{
...
void Scene_Replay::writeActiontoFile(const Action& action, const std::string& filename)
{
	std::ofstream f;
...
void Scene_Replay::loadLevel(const std::string& filename)
{
...
void Scene_Replay::turnPixelsToSFML(float& x, float& y)
{
	y *= 64;
	x *= 64;
	y += 32;
...
void Scene_Replay::update()
{
	...
	if (1)
	{
		...
		if (replay.find(m_scenePlayFrame) != replay.end() && m_scenePlayFrame <= m_scenePlay->getFrame())
		{
			sDoAction(replay[m_scenePlayFrame]);
			std::cout << "\n" << replay[m_scenePlayFrame].name << "\n";
		}
		else if (m_scenePlayFrame > m_scenePlay->getFrame()) {
...
void Scene_Replay::readReplay(const std::string& filename)
{
	std::ifstream f(filename);
	int frameNum;
	std::string aType;
...
void Scene_Replay::handleMovementOtherEntities()
{
...
void Scene_Replay::resolve(Entity* e1, Entity* e2)
{
...
void Scene_Replay::drawGrid()const
{
...
void Scene_Replay::hitGoldBox(Entity* e)
{
...
void Scene_Replay::hitBombBox(Entity* e)
{
	Entity* e2 = m_eM.addEntity("Tile");
	e2->cTransform = new CTransform(e->cTransform);
	e2->cBoundingBox = new CBoundingBox(Vec2(64, 64));
...
void Scene_Replay::gameOver()
{
	m_game->changeScene("GameOver");
}
void Scene_Replay::completed()
{
	m_game->changeScene("LevelComplete");
}
Entity* Scene_Replay::getPlayer() {
	return m_player;
}

void Scene_Replay::resetForReplay() {
}
